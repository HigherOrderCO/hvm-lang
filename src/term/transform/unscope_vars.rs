    ...
    for (name, def) in self.defs.iter_mut() {
      for rule in def.rules.iter_mut() {
        ...
        if !u.errors.is_empty() {
          eprintln!("In def {}", name.0);
        }
        for err in u.errors {
          match err {
            Error::UnboundVar(name) => {
              eprintln!("Unbound variable {}", name.0);
            }
          }
        }
      }
...
impl<'t> Unscoper<'t> {
  ...
  fn push_var(&mut self, name: &'t mut Name) {
    if let Some(e) = self.scope.entry(name.clone()).or_default().last_mut() {
      e.push(name)
    } else {
      self.errors.push(Error::UnboundVar(name.clone()));
    }
  }
